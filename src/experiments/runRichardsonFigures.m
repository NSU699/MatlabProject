...
%   作者: 项目成员   日期: 2026-09-10
    ...
    ax = axes(fig);
    loglog(ax, h, coarseError, 'o-', 'Color', '#354E68', ...
        'LineWidth', 1.8, 'MarkerSize', 6, ...
        'DisplayName', '二阶中心差分');
    hold(ax, 'on');
    loglog(ax, h, richardsonError, 's-', 'Color', '#3B9D9A', ...
        'LineWidth', 1.8, 'MarkerSize', 6, ...
    ...
    plot(ax, summary.coarseN, summary.coarseOrder, 'o-', ...
        'Color', '#354E68', 'LineWidth', 1.8, 'MarkerSize', 6, ...
        'DisplayName', '二阶中心差分');
    hold(ax, 'on');
    plot(ax, summary.coarseN, summary.richardsonOrder, 's-', ...
        'Color', '#3B9D9A', 'LineWidth', 1.8, 'MarkerSize', 6, ...
        'DisplayName', 'Richardson 外推');
    yline(ax, 2, '--', 'Color', '#354E68', ...
        'LineWidth', 1.1, 'DisplayName', 'p=2');
    yline(ax, 4, '--', 'Color', '#3B9D9A', ...
    ...
    semilogy(ax, points.x(interior), points.coarseAbsError(interior), 'o-', ...
        'Color', '#354E68', 'LineWidth', 1.5, 'MarkerSize', 5, ...
        'DisplayName', '二阶中心差分');
    hold(ax, 'on');
    semilogy(ax, points.x(interior), points.richardsonAbsError(interior), 's-', ...
        'Color', '#3B9D9A', 'LineWidth', 1.5, 'MarkerSize', 5, ...
        'DisplayName', 'Richardson 外推');
    hold(ax, 'off');
    xlabel(ax, '空间坐标 x');
    ylabel(ax, '点态绝对误差');
    ...
    ax = axes(fig);
    semilogy(ax, points.x(interior), points.coarseAbsError(interior), 'o-', ...
        'Color', '#354E68', 'LineWidth', 1.5, 'MarkerSize', 2, ...
        'DisplayName', '二阶中心差分');
    hold(ax, 'on');
    semilogy(ax, points.x(interior), points.richardsonAbsError(interior), 's-', ...
        'Color', '#3B9D9A', 'LineWidth', 1.5, 'MarkerSize', 2, ...
        'DisplayName', 'Richardson 外推');
    hold(ax, 'off');
    xlabel(ax, '空间坐标 x');
    ylabel(ax, '点态绝对误差');
    ...
    ax = axes(fig);
    semilogy(ax, points.x(interior), points.coarseAbsError(interior), 'o-', ...
        'Color', '#354E68', 'LineWidth', 1, 'MarkerSize', 1.5, ...
        'DisplayName', '二阶中心差分');
    hold(ax, 'on');
    semilogy(ax, points.x(interior), points.richardsonAbsError(interior), 's-', ...
        'Color', '#3B9D9A', 'LineWidth', 1, 'MarkerSize', 1.5, ...
        'DisplayName', 'Richardson 外推');
    hold(ax, 'off');
    xlabel(ax, '空间坐标 x');
    ylabel(ax, '点态绝对误差');
